...
for j = 1:L-1
    ...
    S(j+1) = S(j) + (lambda-beta*S(j).*I(j)-mu*S(j))*Dt + sigma_1*S(j).*Winc + 0.5*sigma_1^2*S(j)*(Winc.^2 - Dt);

    I(j+1) = I(j) + (beta*I(j)-(mu+epsilon+gamma)*I(j))*Dt + sigma_2*I(j).*Winc+ 0.5*sigma_2^2*I(j).*(Winc.^2 - Dt);

    R(j+1) = R(j) + (gamma*I(j)-mu*R(j))*Dt + sigma_3*R(j).*Winc + 0.5*sigma_3^2*R(j).*(Winc.^2 - Dt);
end
